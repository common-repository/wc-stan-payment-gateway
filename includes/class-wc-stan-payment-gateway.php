...
class WC_Stan_Payment_Gateway extends WC_Payment_Gateway {
    ...
    public function filter_stan_payment( $available_gateways ) {
        ...
        $payment_id = "wc_stan_payment_gateway";
        ...
        $is_stan_app = str_contains( $user_agent, "StanApp" );

        if ( $this->only_stanners && ! $is_stan_app) {
            unset( $available_gateways[ $payment_id ] );
        }

        if ( isset( $available_gateways[ $payment_id ] ) && $is_stan_app ) {
            $available_gateways[ $payment_id ]->set_current();
        }
    ...
    /**
	 * Sets up Stan Connect button
	 *
	 * @since    2.7.6
	 */
    public function setupStanConnect() {
        if ( $this->stan_connect ) {
            ...
            if ( is_plugin_active( 'stan-easy-connect/stan-easy-connect.php' ) || is_plugin_active( 'wp-stan-easy-connect/stan-easy-connect.php' ) ) {
                add_action( 'woocommerce_login_form_start', array( $this, 'display_stan_connect_in_login' ), 5 );
                add_action( 'woocommerce_before_checkout_billing_form', array( $this, 'display_stan_connect_in_checkout' ), 5 );
            } else {
	...
	public function display_stan_connect_in_login() {
        ...
        echo '<h4 class="stan-connect-checkout-text">— Ou se connecter manuellement —</h4>';
		echo '<br />';
	}
    ...
    public function get_icon() {
        $icon = '<img src="' . WC_STAN_PAYMENT_PLUGIN_URL . '/public/images/stan-pay.svg" id="stan-pay" class="icon" alt="Stan payment icon" />';
